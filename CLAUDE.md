...
- **Group admins are NOT auto-enrolled** into the groups they administer. The group-admin role grants **approval rights only** (review / approve / reject requests for that group); it creates no membership and no platform grant. If a group admin needs actual data access they request it through the normal request → approval → duration flow like any other user. The Groups list still shows admins an ACTIVE badge as a cosmetic short-circuit (see `group.controller`), but that is not a real grant. Because no membership is auto-created, **removing a group-admin role only removes approval rights** — there is nothing to revoke.
