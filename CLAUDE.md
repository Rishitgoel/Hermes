...
- Key flags:
  ...
  - `SECRETS_AZURE_SIMULATION=true|false` (backend). Enables/simulates the Azure Key Vault Secret Ingestion instance (`secrets-azure`). `=true` **on its own is a complete local setup** — `KeyVaultService` serves two seeded mock vaults (`bachatt-sim-kv`, `bachatt-sim-kv-analytics`), so the vault picker behaves as it does live; scope a local group with `*` or one of those names. `=false` with **neither** `SECRETS_AZURE_SUBSCRIPTION_ID` nor `SECRETS_AZURE_VAULT_NAME` is a **startup error**, not a silent fallback (writes would look like they succeeded while landing in the mock store).
...
**Secret Ingestion — AWS Secrets Manager Integration (approvals-gated).** The `'secrets'` platform allows staging key-value secrets to AWS Secrets Manager. A secrets-backed group's `externalGroupId` is a newline-separated list of AWS secret names (or wildcard patterns) that the group owns (resolving scope server-side). Users submit requests to stage new key-value pairs; administrators review these requests per-key and apply mix approval/rejection decisions. Approved keys are merged/written to AWS (lazy creating the secret if it doesn't exist), and staged secret values are permanently redacted (nulled out) in the database once the request reaches a terminal status (`APPLIED`, `REJECTED`, or `PARTIALLY_APPLIED`).
- **Multi-secret cart checkout:** Users can queue multiple secrets in a cart and checkout in one action via `POST /api/secrets/requests/bulk` (`createIngestionRequestsBulk`), which fans out to individual requests sharing a `batchId` (sequential to prevent race conditions).
- **Scope patterns are resolved LIVE (no manual re-sync).** Each `externalGroupId` line is a scope pattern (`secretsManagerService.parseScopePatterns`): `*` = every secret in the account, `investments*` = all names starting with `investments`, anything else = an exact name (the original, back-compatible form). Wildcards are expanded on every read against `listAllAwsSecrets()` (a live `ListSecrets`, short-TTL cached ~30s and invalidated on secret creation), so newly-added AWS secrets that match a group's pattern appear automatically — no group edit needed. `secret-ingestion.service.ts`: `resolveUserScopePatterns` (raw, no AWS call) → `resolveUserSecretTargets` (expanded list, for the UI dropdown) / `resolveSecretForUser` (authorization + canonical casing; exact grants win first, then wildcard — a prefix scope may create a brand-new secret in its namespace). Exact-name behavior/casing is unchanged. Frontend ([SecretIngestion.tsx]): the Target AWS Secret uses a **two-stage filter** — a prefix pre-filter (secrets *starting with* the text) feeds the `SearchableSelect`, whose own search does a substring match *within* that pre-filtered set; the Existing Keys panel has its own search + scroll.
- **No-terminal prod setup.** Standing up the wildcard-all group is a super-admin-only endpoint `POST /api/admin/maintenance/ensure-secrets-group` (`adminManagementController.ensureAllSecretsGroup`), idempotent on the *scope* (`externalGroupId: '*'`), not the name — returns the existing group if one is already present; audited `GROUP_CREATED`. There is **no button for this in `AdminManagement.tsx`** (removed deliberately — it's a one-time setup step, and the same group can just be created by hand via "New group" with `externalGroupId: '*'`, same as any other secrets group). The endpoint + `ensureAllSecretsGroup` frontend helper (`services/api/admin.ts`) and the `scripts/create-secrets-group.ts` CLI path still work if ever needed again.
- **Infra-deployment PR auto-merge toggle** (`INFRA_REPO_AUTO_MERGE` / `SECRETS_SANDBOX_INFRA_REPO_AUTO_MERGE`). Controls whether Hermes merges the deployment PR automatically or leaves it open for the DevOps team to review. **Default: OFF (manual).** Both Secret Ingestion (on approval) and Secret Drift ("Solve drift") obey the same toggle per instance — `isInfraAutoMergeEnabled(platform)` in `infra-repo-sync.service.ts`. When **OFF**: the event listener (`event-listeners.ts`) opens the PR but stops at `infraSyncState: OPEN` without calling `mergePrForRequest`. The DevOps team reviews the diff and merges the PR manually on GitHub. The request disappears from the review queue once applied and the PR is successfully opened. ⚠ **The toggle governs Secret Ingestion approvals ONLY — Secret Drift ignores it entirely and is always a two-step, human-gated flow**: "Solve drift" opens the draft PR and stops, and it lands only when someone reviews the diff and clicks **"Merge PR"** (`mergeDrift`). This is deliberate and was asked for explicitly: a drift PR is raised by a *scan*, not proposed by a person, so nobody has looked at the manifest edit when it's opened — auto-merging it would land a diff on the infra repo that no human ever saw. An ingestion PR is different: a reviewer already approved those specific keys, so auto-merge is a reasonable opt-in there. `resolveDrift` therefore does **not** call `isInfraAutoMergeEnabled`, and the "Merge PR" button is never gated on the toggle — don't "restore" either as a consistency fix. `MergeSettingsModal.tsx` spells this scope out in its own copy. When **ON**: ingestion approval merges automatically the moment the PR is ready — the event listener calls `mergePrForRequest` (original behavior, now opt-in). Config: `config.infraRepo.autoMergeEnabled` (prod) / `config.secretsInstances[sandbox].infraRepo.autoMergeEnabled` — same `process.env.*` → boolean pattern as `isSimulation`.

**Secret Ingestion — AZURE KEY VAULT (`secrets-azure`, a second PROVIDER not a second account).** The secrets family spans two backing stores behind one contract, `SecretStore` (`secret-store.interface.ts`): `SecretsManagerService` (`provider: 'aws'`) and `KeyVaultService` (`provider: 'azure'`). `getSecretsManagerService(platform)` picks the implementation off the instance's `provider`; absent ⇒ `'aws'`, so every pre-Azure instance is unchanged.
- **The model.** Key Vault secrets are FLAT (one secret = one string), but Hermes' whole domain model is two-level (secret → keys). So a **VAULT is modelled as a "secret" and that vault's secrets as its "keys"**: `secretName = "bachatt-prod-kv"`, `entries[].key = "orbit-kfin-username"`. Every caller above the store stays provider-agnostic. ⚠ **Consequence, accepted deliberately: the unit of authorization is a WHOLE VAULT** — a user scoped to one can request a write to any service's secret in it and enumerate every secret name. Security rests on the admin approval step. Per-service isolation is not achievable without changing the unit of scope.
- **Two modes, and DISCOVERY is the default.** With `SECRETS_AZURE_SUBSCRIPTION_ID` set and no vault pin, `listAllSecrets()` enumerates **every vault in the subscription** through the Key Vault **management plane** (`@azure/arm-keyvault` → `vaults.listBySubscription()`) — so group scope patterns work across vaults exactly as they do across AWS secrets: `*` = every vault, `bachatt-*` = prefix, a bare name = that one. A vault created in Azure later appears on the next read with no config change. Setting `SECRETS_AZURE_VAULT_NAME` instead **pins** the instance to one vault and never calls ARM (no subscription-level role needed). ⚠ The management plane is a **different API and a different permission** from the secrets data plane: listing vaults needs `Reader` at subscription scope, reading/writing secrets needs `Key Vault Secrets Officer` (or a `get/list/set` access policy). Having one without the other is normal and fails in confusingly different ways — `healthCheck()` deliberately exercises **both**. The data-plane SDK alone *cannot* enumerate vaults, which is why the extra dependency exists. In discovery mode a `*` group reaches every secret in every vault in the subscription — the same blast radius `*` already has on the AWS instances, and an accepted consequence of choosing this mode.
- **Env vars are a second field.** Azure's `secretsStore.mappings[]` needs BOTH the vault secret name (`objectName`) and the env var it lands in (`key`) — unlike AWS `items:`, where the key name IS the env var. `IngestionEntry.envVar` rides along in the existing `entries` JSON column (no migration) and reaches the manifest editor as `keyEnvVars`. Absent ⇒ `deriveEnvVar()` guesses it (drop the leading `<service>-`, uppercase, dashes→underscores). It is only ever a **prefill** — `azure/tolgee` is a live counter-example (`tolgee-db-password` → `SPRING_DATASOURCE_PASSWORD`). ⚠ **Every path that rebuilds the PR branch must pass `keyEnvVars`** — the branch is recomputed from base, so omitting it doesn't skip the overrides, it actively overwrites correct `mappings[].key` lines with derived guesses. `envVarsOfEntries()` (secret-ingestion.service.ts) is the single source; the reviewed-event listener and `retryInfraMerge` both use it.
- **Key names are validated at REQUEST time**, via the optional `SecretStore.validateKeyName` hook (Key Vault: 1–127 chars, letters/numbers/dashes only — **no underscores**). This ordering is load-bearing: the infra PR is opened when the request is *created*, so validating only at write time would leave a merged-ready manifest edit for a key Azure will refuse the moment a reviewer approves.
- **Writes are NOT atomic.** Key Vault has no multi-secret write, so `putSecretKeyValues` issues one `setSecret` per key. A mid-loop failure throws `PartialWriteError` carrying the keys that DID land, and `reviewIngestionRequest` marks exactly those applied. AWS never hits this (one `PutSecretValue`).
- **Reads are per-key.** AWS returns every key in one `GetSecretValue`; Azure needs one `GetSecret` **per value**. `getSecretMap(name, keys?)` takes a narrowing hint that AWS ignores and Azure honours — **callers must pass it** or an approvals-queue render costs one call per secret in the whole vault.
- **Manifest shape.** `azure-values` is a third `Mechanism` alongside `helm-values`/`spc`: one level shallower, because a Key Vault secret has no inner keys — the vault is the "secret" and `mappings[]` IS the key list. Hermes never edits the Azure SecretProviderClass; the shared chart renders both `objects:` and `secretObjects.data` from `mappings`, so editing the values file suffices. Routing reads the instance's declared **`manifestFlavor`**, not the `azure/` path prefix (that's only a fallback for the exported pure helpers).
- **One repo, one-directional scoping.** Azure mirrors the SAME infra-deployment repo as prod, scoped via `pathInclude: 'azure/'` — Azure never sees anything outside `azure/`. Prod is **not** excluded from `azure/` in return (deliberate, accepted trade-off): the real infra-deployment repo has at least one genuinely AWS-shaped manifest (`azure/findesk-be/prod/values-prod.yaml`, an EKS deployment misfiled under `azure/`) that prod would otherwise never discover or be able to register keys into. ⚠ **Known accepted risk**: `manifestRef` on a requester's `infraTargets` is client-supplied and never validated against their own resolved scope (`secret-ingestion.validation.ts`) — a requester with only AWS `secrets` access can point their own request at an Azure-shaped file (orbit/saathi-be/tolgee, all under `azure/`) with a `manifestRef` naming a real vault. Whatever damage that can do depends entirely on `isAzureValuesFile`'s routing (`infra-repo-sync.service.ts`) correctly steering it to the AWS editor rather than falling back to path-based Azure detection — the AWS instances never set `manifestFlavor`, so that fallback is still live. This was a knowing choice to unblock `findesk-be`, not an oversight; revisit if `manifestRef` ever gets validated against scope, or if `manifestFlavor: 'aws'` is made explicit for the AWS instances to close the fallback.
- **Drift is report-only on Azure.** `InfraRepoSyncService.consumersEnumerateAllKeys` is false for `manifestFlavor: 'azure'`, so `missingInManifest` is never computed there and `fixable` is always false. One vault backs orbit, saathi-be and tolgee, each mapping its own subset **by design** — orbit not listing saathi's keys is correct, not drift, and "Solve drift" would otherwise append every vault secret to every service's manifest, mounting unrelated credentials into unrelated pods. Dangling (`missingInAws`) keys and `unmatched` manifests still report.
- **Validated live** (2026-07-21): connectivity, discovery mode (both vaults visible), and the manifest editor were exercised end-to-end against real Azure credentials and a real GitHub-hosted manifest (`dummy-hermes-pipeline`) by the user. The throwaway `secrets-azure-test` instance that existed solely for that validation has been removed from config — it served its purpose and added an extra credential surface with no further use. Two preflight scripts remain for future re-validation (e.g. after a credential rotation or a new vault), both run by hand from `backend/` against the real `secrets-azure` instance:
  - `npx ts-node scripts/check-azure-keyvault.ts [--write] [--instance <key>]` — real service-principal auth, paginated list, narrowed read, and (crucially) that Azure's error objects carry the fields `toAzureError` switches on.
  - `npx ts-node scripts/check-azure-manifests.ts [--key <name>] [--instance <key>]` — READ-ONLY dry-run against the real infra-deployment repo: which manifests consume the vault, what they register, and what the editor *would* add.

**Drift detection (AWS Secrets Manager ⇄ infra-deployment).** `secret-drift.service.ts` compares each in-scope secret's *actual* AWS keys against the keys the infra-deployment manifests *register* (the `items:`/`jmesPath:` lists — the same enumerations the CSI driver syncs to pods). Drift requires **at least one consuming manifest** — a secret no manifest references at all is *not* drift (there's nothing for its keys to disagree with), it's simply not surfaced, so `driftForSecret` short-circuits (`manifests.length > 0` gate) before even looking at key sets. Two drift kinds per secret, both requiring ≥1 consuming manifest: **missingInManifest** (key in AWS, not enumerated in that manifest → pods can't see it — *fixable*) and **missingInAws** (enumerated in a manifest, absent from AWS → dangling — report-only); plus `unmatchedManifests` (referenced but key-list shape unparseable). The registered-key extraction lives in `infra-repo-sync.service.ts` (`registeredKeysInFile` + the `resolveDrift` method — the read-only flip side of the `editValuesItems`/`editSpc` editors, reusing the cached consumer index). Scope: `detectDrift(user, platform)` uses `getManageableGroupIds` (super/platform admin → every secrets group on the platform; group admin → their groups), expands each group's scope patterns against `listAllAwsSecrets`, and caps the scan at `MAX_DRIFT_SECRETS = 500` (logged, never silent). **Solve, then Merge — always both steps, regardless of the auto-merge toggle (which is ingestion-only; see toggle docs above).** The draft PR is opened, a human reviews the diff, and it merges only on an explicit "Merge PR" click — from Hermes or on GitHub. `POST /api/secrets/drift/resolve` reuses `openPrForRequest` with a stable per-secret `requestId: 'drift'` (idempotent branch `hermes/secret-keys/<safe>-drift`) to open a **draft** PR registering the missing keys. Once a human has reviewed it in GitHub, `POST /api/secrets/drift/merge` (`secretDriftService.mergeDrift` → `infraRepoSyncService.mergeDriftPr`) looks the PR up live by its deterministic branch name (there's no DB row carrying the PR number for a drift PR, unlike an ingestion request), **recomputes the currently-missing keys fresh** (not the stale scan snapshot) so it always merges what's actually still missing, marks the draft ready, and squash-merges via the same `mergePrForRequest` the ingestion-approval flow uses — audited `SECRET_DRIFT_MERGED` (written whichever way it lands; `details.prState` carries the real outcome). **A GitHub refusal is not an error**: `mergePrForRequest` catches it, comments on the PR, and returns `state: 'FAILED'` with GitHub's own reason, so `/drift/merge` answers 200 and the card renders the reason plus a "Merge on GitHub" link and a retry — the PR is untouched and still mergeable by hand. `mergePull` translates the two dead-end statuses itself: **405** (draft / branch protection / pending check / conflict — retried 4× first, since it's often transient mergeability) and **403/404** (the token lacks merge rights, or can't see a private repo — never clears on retry, so the message points at the credential). Because `mergeDrift` refuses to call through with an empty key list, it can never hit `prepareApprovedBranch`'s `approvedKeys.length === 0` branch, which would **close** the PR rather than merge it. `GET /api/secrets/drift` returns the report. All three are `authenticateToken` only, scoped in the service (empty when the caller manages nothing). **Manual only — no scheduled scan, no notifications.** Drift is entirely on-demand: an admin clicks "Scan for drift", reads the report, and Solves/Merges. There is **no cron job** and Hermes **never notifies** anyone about drift (the former scheduled `scanAllAndNotify` + `notificationService.notifySecretDriftDetected` + the `SECRET_DRIFT_ALERTED` fingerprint dedupe were all removed — a drift PR is raised by a person looking at the report, so an alert path added nothing). Simulation makes it demoable offline: `resolveDrift` returns a representative manifest registering all-but-one AWS key, and `mergeDriftPr` fakes a MERGED result with no GitHub calls. UI: `frontend/src/components/secrets/SecretDriftPanel.tsx`, mounted on `PendingApprovals.tsx` under `canReviewSecrets`. An on-demand **"Scan for drift"** button (a scan reads AWS + GitHub, so it never auto-runs on mount), then a per-secret **"Solve drift"** button (only ever opens the PR) which becomes **"Merge PR"** once a PR is waiting. That button survives a reload because `SecretDrift.openPr` carries the open PR found live by `attachOpenPrs` (`infraRepoSyncService.findDriftPr`) — solving and merging are separated by a GitHub review, so they rarely share a session, and a drift PR has no DB row to read it from. The lookup is **interactive-only** (`computeDrift({ withOpenPrs })`, set by `detectDrift`) and runs only for `fixable` drifts, keeping it to a handful of calls even on a wildcard-all scan; it's best-effort, so a rate limit costs the button, never the report. Simulation returns no `openPr` (there's no real PR to find), so the sim Solve→Merge demo runs off the panel's own in-session state. **Dangling (missingInAws) keys are report-only** — rendered on the on-demand report as strikethrough chips with no per-key action. (There used to be an ignore/unignore mute — `SECRET_DRIFT_KEY_IGNORED`/`_UNIGNORED` audit rows + `loadIgnoredKeys` + `SecretDrift.missingInAwsIgnored` + `/drift/ignore`|`/unignore` — but it existed *only* to quiet the scheduled notifications, so it was removed along with them. Don't re-add it without also re-adding a notification path.) `fixable`/missingInManifest still drives the Solve/Merge PR flow, unchanged.
...
#### Withdrawing an open request (requester-initiated)

A requester can pull back their **own** still-open request across all three request types. Each has a
`WITHDRAWN` value on its status enum — deliberately **not** reused `REJECTED`, so the audit trail never
reads a self-withdrawal as an admin decision (reviewer fields stay null; the audit row's performer is the
requester). Endpoints are all `POST .../:id/withdraw`, `authenticateToken` only, with **ownership** as the
authorization — the service 404s (not 403s) a request the caller didn't file, so a non-participant can't
probe for one. Withdrawal is **not** an admin capability: an admin ending someone else's request is a
rejection, which already has its own path. Each uses a conditional `updateMany` as the race guard against
an admin reviewing at the same instant (the review paths claim the row the same way), so the loser gets a
`ConflictError` instead of silently withdrawing an already-approved request.

| Request | Endpoint | Withdrawable from | Side effects |
|---|---|---|---|
| `AccessRequest` | `/api/access-requests/:id/withdraw` | `PENDING`, `WAITING_FOR_SETUP` | none |
| `ZookeeperChangeRequest` | `/api/zookeeper/requests/:id/withdraw` | `PENDING` only | none |
| `SecretIngestionRequest` | `/api/secrets/requests/:id/withdraw` | `PENDING` only | values redacted; infra PR closed |

- **Access requests include `WAITING_FOR_SETUP`** (approved, but queued behind platform-account setup — nothing
  provisioned yet, so there's nothing to undo). Those are exactly the two statuses the one-open-request-per-
  `(user, group)` partial unique index covers, so **withdrawing is also how a user unblocks themselves** to
  re-request a group whose stale request was stuck. WITHDRAWN falls out of that index automatically — the
  index needed no change.
- **ZK and secrets stop at `PENDING`, deliberately narrower than their review paths** (which also accept
  `APPLY_FAILED`). An `APPLY_FAILED` request is already approved and may have half-written znodes, or — on
  Azure Key Vault, which writes one key at a time — keys already live in the vault. Letting the requester
  withdraw would bury partial writes behind a status claiming nothing happened. Recovering those is the
  reviewer's job.
- **Secret ingestion is the only one with an external side effect.** Values are redacted exactly as for
  `REJECTED`, and the infra-deployment PR (opened at **submit** time, before any review) is closed by the
  `secret-ingestion.withdrawn` listener — best-effort and decoupled, mirroring how it was opened. Because
  that PR opens *asynchronously*, a fast withdraw can interleave with it; the submit listener therefore
  both skips opening for an already-`WITHDRAWN` row **and** re-checks after `openPrForRequest` returns,
  closing the PR it just made. Without both checks a withdraw-right-after-submit leaves a dangling draft PR.
- UI: a **Withdraw** button + `ReasonModal` (reason optional) on each of the three "my requests" tables —
  `MyRequests.tsx`, `ZookeeperConfig.tsx`, `SecretIngestion.tsx`. WITHDRAWN badges render neutral
  (`badge-revoked`), not red.
...
- ✅ **Secret Ingestion platform live** (`secrets` platform/family): approvals-gated AWS Secrets Manager integration; supports single-secret requests and multi-secret checkout via bulk endpoint (`/requests/bulk`); approvals support an auto-merge toggle (`INFRA_REPO_AUTO_MERGE`) per instance.
- ✅ **Azure Key Vault provider (`secrets-azure`) — validated live (2026-07-21).** Second `SecretStore` implementation behind the same contract; `azure-values` manifest editor; env-var (`mappings[].key`) support; drift is report-only there. Real SDK auth (both management + data plane), multi-vault discovery, error mapping, and the real `azure/**` manifest editor were all exercised against live Azure credentials and a real GitHub-hosted manifest by the user. The `secrets-azure-test` throwaway instance used for that validation has since been removed. Re-run `scripts/check-azure-*.ts` against the real `secrets-azure` instance if credentials, vaults, or manifests change materially — e.g. before porting to admin-panel.
- ✅ **Secret Drift detection live (manual only)**: compares live AWS Secrets Manager key lists against registered keys in infra deployment manifests; **on-demand scan only — no scheduler job, no notifications**; surfaces Solve drift (opens PR) and Merge PR flows. Dangling (missingInAws) keys are report-only.
