...
**ZooKeeper — multi-path groups (editable, with member reconciliation).** ZooKeeper has no native group object (a znode is just a *path + ACL*), so a "ZK group" is a Hermes abstraction: its `externalGroupId` is a **newline-separated list of `path#perms` entries** (a single line = the original one-path form). `zookeeper.provisioner.ts` fans a grant out to one `digest` ACL entry per path (`zookeeperService.parseExternalGroupIds`), and `deprovision` strips all of them. Because the path list is *editable* (unlike Redash/AWS's single immutable group id), editing it must update **existing** members: the new optional adapter hook **`reconcileMembers(ctx)`** (in `provisioner.interface.ts`) diffs old vs new paths by path and grants added/perms-changed paths to every active member + removes dropped ones (best-effort; the saved config is the source of truth, per-member failures are returned + audited). The admin layer calls it from `updateGroup` / `updateGroupLevel` when `externalGroupId` changes and audits **`GROUP_PATHS_RECONCILED`**; **implementing the hook is what opts an adapter's `externalGroupId` into being editable** — `updateGroup` rejects the edit for platforms without it (Redash/AWS stay immutable). UI: an editable paths textarea in the group detail drawer's Settings tab (`GroupSettingsTab.tsx`) and the Levels tab form (`GroupLevelsTab.tsx`). Adding a path does **not** retro-grant via sync — reconciliation happens at edit time. Because levels of a group will often **share paths**, an admin-set-level swap (`_swapGrant`) passes `DeprovisionContext.retainExternalGroupId` (the new mapping) so the old-mapping deprovision **keeps any path the new level still grants** — shared paths survive the swap with the new level's perms (single-target adapters ignore the hint). Tests: `zookeeper.provisioner.test.ts`.

**Subtree-read expansion (browsability in ZooNavigator).** ZK ACLs are **per-node, not inherited**, so a leaf grant alone leaves a tree UI unable to *navigate to* the node (can't list its ancestors) or *read into* it (children have their own ACLs). Each grant therefore also lays a `digest` **READ** entry on every **ancestor** (up to `/`, so the tree expands from root) and every **existing descendant** of the granted path — `zookeeperProvisioner.grantSubtree` over `zookeeperService.ancestorPaths` (pure string math, incl. `/`) + `descendantPaths` (recursive walk; sim = prefix scan). The READs are added with the new `addAclEntry(..., { merge:true })` option (ORs perms, never downgrades a stronger grant on a shared node). `revokeSubtree` removes them on revoke but **never strips a node another still-active grant of the same user needs** — a node is "still needed" when it's on the same root-to-leaf line (`onSameLine`) as one of the user's remaining explicit paths; a revoked path that a surviving deeper/parent grant still needs is **downgraded to READ** rather than removed. The cache (`platform_external_users.external_group_ids`) stores **only the explicit granted paths**; ancestor/descendant READs are recomputed from the live tree each grant/revoke, never cached. ⚠ A child znode created **after** a grant is not retro-covered (no inherited ACL) — it stays admin-only until the next reconcile/re-grant. Live UI = **ZooNavigator** (per-user digest auth on its connect screen); **zkui won't work** (single shared connection bypasses per-user ACLs). Tests: the `subtree expansion:` cases in `zookeeper.provisioner.test.ts`.
