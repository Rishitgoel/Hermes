...
- **No-terminal prod setup.** Because prod has no terminal, standing up the wildcard-all group is a super-admin-only endpoint `POST /api/admin/maintenance/ensure-secrets-group` (`adminManagementController.ensureAllSecretsGroup`), surfaced as a **"Set up All-Secrets group"** button on `AdminManagement.tsx` (shown when `superAdmin && activePlatform === 'secrets'`). Idempotent on the *scope* (`externalGroupId: '*'`), not the name — returns the existing group if one is already present; audited `GROUP_CREATED`. The `scripts/create-secrets-group.ts` CLI path still works for local/dev.
