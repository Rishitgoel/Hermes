/**
 * One-shot importer: backfill existing Redash accounts + group memberships into
 * Hermes so users keep the access they already have, without re-requesting.
 *
 * WHAT IT DOES (per real Redash user, after refreshing the live cache):
 *  1. Resolves the user's Hermes identity (Keycloak `sub`) from their email.
 *     A Redash user with no Keycloak account can't own a Hermes grant, so they
 *     are SKIPPED and reported (you decide whether to onboard them to Keycloak).
 *  2. Upserts a COMPLETED UserCreationRequest for (user, redash) so the account
 *     gate is satisfied — they never see the "create your Redash account" flow.
 *  3. For each of their Redash group memberships, maps the Redash group id to the
 ...
 *     already in that Redash group; we are only recording the existing reality.
 *
 * SAFETY:
 ...
 *  - Redash must be LIVE (real REDASH_API_KEY, REDASH_SIMULATION not "true") — else
 *    the "users" are the in-process mock.
 *  - Keycloak must be LIVE (admin credential set) — needed to resolve email → sub.
 *
 * RUN (from backend/):
 *    npx ts-node scripts/import-redash-memberships.ts          # dry run
 *    npx ts-node scripts/import-redash-memberships.ts --apply  # write
 */
...
async function main(): Promise<void> {
  ...
  logger.info(
    `🚚 Redash membership import — ${APPLY ? 'APPLY (writing)' : 'DRY RUN (no writes)'}`,
  );

  // ── Guards ──────────────────────────────────────────────────────────────────
  if (!provisioningRegistry.has(PLATFORM)) {
    logger.error(
      'Redash is not registered in the provisioning registry. Please configure REDASH_BASE_URL.',
    );
    process.exit(1);
  }
  const redashProvisioner = provisioningRegistry.get(PLATFORM);
  ...
  logger.info(
    'Refreshing Redash groups + users and reconciling Hermes groups…',
  );
  ...
  logger.info(
    `Mapped ${targetByExtId.size} Redash group id(s) to Hermes groups/levels.`,
  );
  ...
  if (report.usersSkippedDisabled.length) {
    logger.warn(
      `Skipped ${report.usersSkippedDisabled.length} DISABLED Redash user(s): ${report.usersSkippedDisabled.join(', ')}`,
    );
  }
  if (report.usersSkippedNoKeycloak.length) {
    logger.warn(
      `Skipped ${report.usersSkippedNoKeycloak.length} user(s) with NO Keycloak identity: ${report.usersSkippedNoKeycloak.join(', ')}`,
    );
  }
  if (report.membershipsUnmapped.length) {
    logger.warn(
      `${report.membershipsUnmapped.length} membership(s) had no matching Hermes group/level:`,
    );
...
main().catch(err => {
  ...
  console.error('Redash membership import failed:', err);
  process.exit(1);
});
