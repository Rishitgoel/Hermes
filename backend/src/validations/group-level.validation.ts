...
// so only super/platform admins set it (enforced in the controller).
export const createGroupLevelSchema = z.object({
  ...
  permission: z.string().trim().max(100, 'Permission label must not exceed 100 characters').optional(),
  externalGroupId: z.string().trim().min(1).max(255).optional(),
  rank: z.number().int().min(0, 'Rank must be a non-negative integer').optional(),
