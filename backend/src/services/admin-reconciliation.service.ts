...
export class AdminReconciliationService {
  ...
  async reconcileAll(
    opts: { dryRun?: boolean } = {},
  ): Promise<{ dryRun: boolean; platformAdmins: ReconcileCounts; groupAdmins: ReconcileCounts }> {
    ...
    if (!(await keycloakAdminService.canConnect())) {
      logger.warn('🔁 Admin reconciliation skipped — Keycloak Admin API unreachable.');
      return { dryRun, platformAdmins: zero(), groupAdmins: zero() };
    }
    ...
    const drift = platformAdmins.added + platformAdmins.removed + groupAdmins.added + groupAdmins.removed;
    ...
    if (drift > 0) logger.warn(line + (dryRun ? ' (drift detected)' : ' (drift repaired)'));
    else logger.info(line + ' (in sync)');
  ...
  private async reconcileGroupAdmins(dryRun: boolean): Promise<ReconcileCounts> {
    ...
    for (const g of groups) {
      try {
        const kcIds = new Set(await keycloakAdminService.getUsersInRole(groupAdminRole(g.platform, g.slug)));
        const mirror = await prisma.groupAdmin.findMany({ where: { groupId: g.id } });
        const mirrorIds = new Set(mirror.map((m) => m.userId));
        counts.checked += 1;
        ...
        for (const userId of kcIds) {
          ...
          if (!dryRun) {
            ...
            await prisma.groupAdmin.upsert({
              ...
              update: { userName: p.userName, userEmail: p.userEmail },
              create: { groupId: g.id, userId, userName: p.userName, userEmail: p.userEmail, assignedBy: 'reconcile' },
            });
          }
          counts.added += 1;
          logger.warn(`🔁 Reconcile: ${tag} add missing group_admin mirror (${userId} / ${g.slug}).`);
        }

        for (const m of mirror) {
          if (kcIds.has(m.userId)) continue;
          if (!dryRun) await prisma.groupAdmin.delete({ where: { id: m.id } });
          counts.removed += 1;
          logger.warn(`🔁 Reconcile: ${tag} remove stale group_admin mirror (${m.userId} / ${g.slug}).`);
        }
      } catch (err: any) {
        logger.error(`🔁 Reconcile: group-admin reconcile failed for "${g.slug}": ${err.message}`);
      }
    }
