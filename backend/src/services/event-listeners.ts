...
export function registerEventListeners(): void {
  ...
  eventBus.on('requests.bulk.created', async event => {
    try {
      ...
      await notificationService.notifyRequestsCreatedBulk(
        requesterName,
        duration,
        items,
      );
      ...
      logger.error(
        'Failed to notify requests.bulk.created event:',
        err.message,
      );
    }
  });
  ...
  eventBus.on('request.approved', async event => {
    try {
      const { requesterId, groupName, reviewerName, note, requesterEmail } =
        event.payload as any;
  ...
  eventBus.on('access.revoked', async event => {
    try {
      const { userId, userEmail, groupName, revokerName, reason } =
        event.payload as any;
  ...
  eventBus.on('access.expired', async event => {
    try {
      ...
      await notificationService.notifyAccessExpired(
        userId,
        groupName,
        userEmail,
      );
  ...
  eventBus.on('access.expiring', async event => {
    try {
      ...
      await notificationService.notifyAccessExpiringSoon(
        userId,
        groupName,
        userEmail,
        expiresAt,
  ...
  eventBus.on('access.expiry-failed', async event => {
    try {
      const { userAccessId, userName, groupName, attempts, error, platform } =
        event.payload as any;
      await notificationService.notifyExpiryFailed(
        userAccessId,
        userName,
        groupName,
        attempts,
  ...
  eventBus.on('access.queued-for-setup', async event => {
    try {
      ...
      logger.error(
        'Failed to notify access.queued-for-setup event:',
        err.message,
      );
    }
  });
  ...
  eventBus.on('user-creation.submitted', async event => {
    try {
      const { requestId, userName, userEmail, justification, platform } =
        event.payload as any;
      await notificationService.notifyUserCreationSubmitted(
        requestId,
        userName,
        userEmail,
        justification,
      ...
      logger.error(
        'Failed to notify user-creation.submitted event:',
        err.message,
      );
    }
  });
  ...
  eventBus.on('user-creation.invited', async event => {
    try {
      const { userId, userEmail, reviewerName, platform } =
        event.payload as any;
      await notificationService.notifyUserCreationApproved(
        userId,
        userEmail,
        reviewerName,
        platform,
      ...
      logger.error(
        'Failed to notify user-creation.invited event:',
        err.message,
      );
    }
  });
  ...
  eventBus.on('user-creation.rejected', async event => {
    try {
      ...
      await notificationService.notifyUserCreationRejected(
        userId,
        reviewerName,
        note,
        userEmail,
      ...
      logger.error(
        'Failed to notify user-creation.rejected event:',
        err.message,
      );
    }
  });
  ...
  eventBus.on('user-creation.completed', async event => {
    try {
      const { userId, userEmail, platform, onboardingDetails } =
        event.payload as any;
      await notificationService.notifyUserCreationCompleted(
        userId,
        userEmail,
        platform,
        onboardingDetails,
      ...
      logger.error(
        'Failed to notify user-creation.completed event:',
        err.message,
      );
    }
  });
  ...
  eventBus.on('user-creation.completed', async event => {
    try {
      const { userId, userName, userEmail, externalUserId, platform } =
        event.payload as any;
      await ensureDefaultGroupMembership(platform, {
        userId,
        userName,
        userEmail,
        externalUserId,
      ...
      logger.error(
        'Failed to grant default group membership on user-creation.completed:',
        err.message,
      );
    }
  });
  ...
  eventBus.on('zk-change.submitted', async event => {
    try {
      ...
      await notificationService.notifyZkChangeRequestCreated(
        requestId,
        groupIds,
        groupNames,
        requesterName,
  ...
  eventBus.on('zk-change.reviewed', async event => {
    try {
      ...
      await notificationService.notifyZkChangeRequestReviewed(
        requesterId,
        requesterEmail,
        groupNames,
        status,
  ...
  eventBus.on('secret-ingestion.submitted' as any, async event => {
    try {
      ...
      await notificationService.notifySecretIngestionSubmitted(
        requestId,
        groupId,
        groupName,
        secretName,
      ...
      logger.error(
        'Failed to notify secret-ingestion.submitted event:',
        err.message,
      );
    }
  });
  ...
  eventBus.on('secret-ingestion.reviewed' as any, async event => {
    try {
      ...
      await notificationService.notifySecretIngestionReviewed(
        requestId,
        secretName,
        status,
        reviewerName,
      ...
      logger.error(
        'Failed to notify secret-ingestion.reviewed event:',
        err.message,
      );
    }
  });
  ...
  eventBus.on('secret-ingestion.reviewed' as any, async event => {
    try {
      ...
      const {
        ...
        status,
        platform,
        newApprovedKeys: newApprovedKeysFromEvent,
      } = event.payload as any;
      const instance = platform ?? 'secrets';
      // Each instance mirrors to its OWN infra-deployment repo; skip when that repo isn't wired
      ...
      if (status === 'APPLIED' || status === 'PARTIALLY_APPLIED') {
        ...
        // Auto-merge ON: merge immediately, matching the original always-merge behavior (now
        // opt-in). Auto-merge OFF (default): recompute the branch down to the approved keys
        // and mark it ready, leaving the merge to a human on GitHub. Both paths MUST recompute
        // — the branch was opened at submit time with every PROPOSED key, so a PR that is only
        // marked ready would still register the rejected ones once merged.
        const review = {
          rejectedKeys,
        ...
        const closed = await infra.closePrForRequest({
          request: row,
          reason: row.reviewNote || 'all keys rejected',
        });
        await persistInfraResult(requestId, closed);
      } else if (status === 'APPLY_FAILED') {
        const noted = await infra.notePrFailure({
          request: row,
          error: row.applyError || 'apply failed',
        });
        await persistInfraResult(requestId, noted);
      }
    } catch (err: any) {
      logger.error(
        'Failed to sync infra-deployment PR for secret ingestion review:',
        err.message,
      );
      ...
      try {
        ...
        logger.error(
          'Also failed to record deployment PR FAILED state:',
          persistErr.message,
        );
      }
    }
